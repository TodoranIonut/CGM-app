...
def train_network_model(trainData, savingFile):
    ...
    regressor.add(Bidirectional(LSTM(units=40, return_sequences=True, input_shape=(X_train.shape[1], 1))))
    regressor.add(Dropout(0.2))
    # # Second LSTM layer
    # regressor.add(Bidirectional(LSTM(units=50, return_sequences=True)))
    # regressor.add(Dropout(0.2))
    # Third LSTM layer
    regressor.add(Bidirectional(LSTM(units=40, return_sequences=True)))
    regressor.add(Dropout(0.2))
    # Fourth LSTM layer
    regressor.add(Bidirectional(LSTM(units=40)))
    regressor.add(Dropout(0.2))
    # The output layer
